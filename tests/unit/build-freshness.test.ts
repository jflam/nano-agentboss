...
describe("build freshness", () => {
  ...
  test("detects dirty files newer than a dirty installed binary", () => {
    ...
    expect(status.outOfDate).toBe(true);
    expect(status.reason).toContain("newer changes in cli.ts");
  });
  ...
  test("parses git status paths and filters build-relevant files", () => {
    ...
    expect(paths).toEqual(["cli.ts", "plans/2026-04-02-note.md", "src/new.ts"]);
    expect(paths.filter(isBuildRelevantRepoPath)).toEqual(["cli.ts", "src/new.ts"]);
  });
});
