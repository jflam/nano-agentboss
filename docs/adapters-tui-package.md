...
- `run-tty.ts`: public interactive-TTY capability guards
...
- `theme-ansi.ts`: reusable ANSI foreground/background/attribute styling
  primitives
- `theme-base.ts`: base text, select-list, and markdown theme construction
  helpers
...
- code simplification applied:
  ...
  - split base text, select-list, and markdown theme construction out of the
    TUI theme constructor
  ...
  - split app-level terminal input listener and keybinding dispatch wiring out
    of the TUI app
  - split app-level editor/input listener binding orchestration out of the
    TUI app
