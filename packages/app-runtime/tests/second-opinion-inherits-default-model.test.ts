...
const MODEL_AWARE_AGENT_PATH = fileURLToPath(new URL("../../../tests/fixtures/model-aware-mock-agent.ts", import.meta.url));
  ...
  test("inherits the current default model for the first pass", async () => {
    ...
    writeExecutable(
      ...
      buildWrapperScript({
        agentId: "first-pass",
        logPath,
        targetScript: MODEL_AWARE_AGENT_PATH,
      }),
    );
    writeExecutable(
      codexCommand,
    ...
    try {
      ...
      try {
        ...
        const store = new SessionStore({
          ...
          cwd: process.cwd(),
          rootDir: join(tempDir, "session-store"),
        });
